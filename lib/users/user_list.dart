  ...
  List<Widget> _buildTiles() {
    ...
    if (this.users != null && this.users.length > 0) {
      this.classes.forEach((classe) {
        ...
        var users = this.users.where((user) =>
            user.classUUID == classe.uid || user.getClass(_user.uid) == null);
        var lu = users.length;
        users.forEach((user) {
          children.add(UserCard(
            loans: this.loans,
            user: user,
            onDelete: this._showDeleteDialog,
          ));
  ...
  Widget _manageDisplay() {
    if (users.length > 0) {
      return new Container(
          ...
          child: ListView(
              padding: const EdgeInsets.all(8), children: this._buildTiles()));
    } else {
